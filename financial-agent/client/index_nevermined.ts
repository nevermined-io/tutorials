...
async function askAgent(
  ...
  accessToken: string,
  sessionId?: string
): Promise<{ output: string; sessionId: string; redemptionResult?: any }> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        throw new Error(
          "Payment Required - insufficient credits or subscription"
        );
      }
      throw new Error(
  ...
  for (let i = 0; i < DEMO_CONVERSATION_QUESTIONS.length; i++) {
    ...
    try {
      ...
      if (result.redemptionResult) {
        console.log(
          `💰 Credits redeemed: ${result.redemptionResult.creditsRedeemed || 0}`
        );
      }
