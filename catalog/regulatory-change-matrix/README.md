...
2. Acquire a current Nevermined v2 API key and place it in a private `~/.nvm-router-buyer.json` file, as shown in the [SEC tutorial](../sec-earnings-evidence-pack/README.md). Run `chmod 600 ~/.nvm-router-buyer.json`. The runner creates a new `erc4337` USDC delegation with a cap of at most **$0.25**, a ten-minute expiry and at most `1 + --max-resolves` transactions. The buyer must have funds on Live Tempo; a card delegation does not work for MPP/Tempo.
...
```sh
# Use the private ~/.nvm-router-buyer.json file from step 2.
...
Use a unique `--run-id` for each intended purchase. For the same run, the script derives stable `X-Router-Request-Id` values for changes and every citation resolution. The new delegation ID is saved privately in `output/private-delegation.json`. A dropped response or 409 stops the run: inspect `GET /api/v1/router/payments?delegationId=...` before retrying. Do not change the run ID to defeat deduplication. The sample writes no secret to the artifact.

The default host is the production `.app` live API. Set `NVM_API_BASE=https://api.live.nevermined.dev` only if your API key and wallet belong to the `.dev` staging-live deployment, and use that same host in the private buyer file. The script accepts only these two exact live API hosts.
...
The example's $0.16 vendor charge and $0.192 planned ceiling are **estimates**, not observed spend. `actual_spend_usd` appears only after a successful live run and is derived from the Router payment ledger's recorded atomic amount, asset decimals, and fee cents. The script accepts `Issued` or `Settled` in the Router header but requires a matching **Settled merchant** ledger record. It briefly polls buyer-fee status: `Accrued` or `Submitted` is disclosed in `buyer_fee_reconciliation_pending`, while `Released` is terminal with no collected fee. An unknown or failed fee state stops the run. The time to first successful result is unobserved.
...
- Delegation creation fails: check the private buyer key, funding and `erc4337` eligibility.
...
For review of current source truth, use [GovLaws machine-payment docs](https://govlaws.ai/mpp), [its OpenAPI](https://govlaws.ai/openapi.json), and the [public Nevermined Router overview](https://nevermined.ai/router/). The slug-routing request shape is demonstrated in [run.py](run.py). See [FRICTION.md](FRICTION.md) for the reproducible Catalog gaps found while preparing this sample.
