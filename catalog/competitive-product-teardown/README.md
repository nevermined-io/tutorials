...
```sh
export NVM_API_KEY='your-current-v2-key'
...
The script makes POSTs to `/api/v1/router/route` using `slug` and `path` for Brave, Serper, and optional DeepSeek. The Router envelope contains the new delegation ID and a stable `requestId` derived from `--run-id` and service. Use a new run ID only for a genuinely new purchase. A 409, timeout, missing payment ID, empty response, or unexpected body shape stops the run. Inspect `GET /api/v1/router/payments?delegationId=...` before any retry. Never change the request ID to evade a duplicate.
...
The runner saves each raw Router envelope and any HTTP error body in private `out/private-*-router.json` files with mode 0600; `out/` is ignored by Git. These diagnostic files can contain vendor content and must stay private. It also stores a private run-state file after the first valid Brave result with its UTC timestamp and elapsed seconds from command start, so a later paid failure does not erase time-to-first-result evidence. A successful public artifact includes those fields without the private responses. If DeepSeek returns empty `choices[0].message.content`, the runner reports `finish_reason` and `usage` from the paid response and stops; do not retry with a fresh request ID merely to fill the missing text.
