...
After that fix, a complete fresh run of `./run-demo.sh` passed all five purchases without manual steps. I repeated once with timing and cost fields instrumented; that run initially supplied the public sample. Every call returned a delivered 200 upstream body, `paid:true`, and a Settled MPP/Tempo merchant payment. The five responses used the `body.data` wrapper, which the pack builder now records. No service was substituted during the final run.
...
A subsequent complete Apple (`CIK 320193`, `AAPL`) run again delivered all five paid responses and wrote `out/evidence.md` and `out/evidence.json`. The saved ledger has five Settled merchant payments at $0.008 each, for **$0.040** in merchant charges. The delegation consumed **5¢** of its **10¢** cap. The saved timing summary records **10 seconds** to the first result and **41 seconds** for the full run. The checked-in sample has now been rebuilt from this follow-up to distinguish its three-month XBRL revenue (**$109.417 billion**) and net income (**$29.789 billion**) from the earlier nine-month values. Its EDGAR search route is the slug base (`path: ""`), and the search count of 10,000 is a lower bound (`relation: gte`). Raw Router envelopes and payment records remain in ignored `out/`.
