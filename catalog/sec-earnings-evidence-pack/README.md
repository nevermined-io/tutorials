...
The pack chooses the latest 10-K/10-Q in the SEC submissions response, recent XBRL revenue and net-income facts, the EDGAR search hit count, and Alpha Vantage quarterly EPS and income fields. It keeps period end, filing date, and accession where available so figures can be checked against the actual filing. EDGAR full-text hits can include third-party mentions. XBRL concepts and Alpha Vantage reports can use different periods, so the pack does not imply they reconcile.
